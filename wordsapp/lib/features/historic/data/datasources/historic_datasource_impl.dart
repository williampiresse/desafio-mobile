...
import 'historic_datasource_interface.dart';

class HistoricDatasourceImpl implements HistoricDatasourceInterface {
  ...
  Future<ApiResponse<List<WordModel>>> getHistoric([int page = 1]) async {
    ...
    final response = await _httpClient.get(
        "${Url.dictionary}/v1/histories?page=$page&limit=25",
        headers: headers);

    if (response.statusCode == 200) {
      final jsonParse = jsonDecode(response.body);
      final data = jsonParse["data"] as List;
      final statusCode = jsonParse["status_code"];
  ...
  Future<bool> save(WordModel word) async {
    ...
    final response = await _httpClient.post(
        "${Url.dictionary}/v1/history/create",
        headers: headers,
        body: params);
